  ...
  return (
    <div className=" h-full ">
      <div className="flex flex-col gap-4  h-full">
        <div className="absolute top-36 right-1 z-[1000]">
          {/* {!blobState.isAnimating ? ( */}
          <Button
            onClick={cycleBlobStates}
            disabled={blobState.isAnimating}
            className="mt-4 p-2 border rounded-md max-w-[200px] pointer-events- "
          >
            Click
...
export function DynamicIslandDemo() {
  return (
    <DynamicIslandProvider initialSize={"default"}>
      <div className="z-[1000] absolute top-0 right-0 left-0">
        <DynamicAction />
      </div>
    </DynamicIslandProvider>
  )
